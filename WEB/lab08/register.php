<?php
    ...
    && isset($_POST['user']) && isset($_POST['pass']) && isset($_POST['pass-confirm']))
    {
        ...
        $sql = "SELECT COUNT(*) AS `count` FROM `account` where `username` = '$user' OR `password` = '$email'";
        ...
        if (empty($first_name)) {
            ...
            if($count == 1){
                ...
                $sql = "INSERT INTO `account` (`username` ,`firstname`, `lastname`, `email`, `password`) VALUES ('$user', '$first_name', '$last_name', '$email', '$pass')";
