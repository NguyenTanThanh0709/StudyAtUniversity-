...
void countDepartment(char *output, NV nv[], int depart){
    FILE *Fout;
    Fout = fopen(output,"w");
...
void reportN(char *output, PG pg[],float n){
    FILE *Fout;
    Fout = fopen(output,"w");
   
    int i;
    int temp[SOLUONG], j = 0;
    for(i = 0; i < SOLUONG; i++){
        if(pg[i].progress == n){
...
void countryName(char *output, NV nv[], char *country){
    FILE *Fout;
    Fout = fopen(output,"w");
...
void averageX(char *output, PG pg[], int x){
    FILE *Fout;
    Fout = fopen(output,"w");
...
int main(){
    ...
    if(strstr(cauLenh,count) != NULL){
        ...
        if(countSpace(cauLenh) == 1){
            ...
            countDepartment(output,nv,department);
        }else{
            fileWrong(fileLoi);
        }
